...
#define ADDR_INC 0x00010000
#define ADDR_RANGE 0x00001000
...
unsigned char *Quantize(float *input, int img_size, int amount, wlt_header_info &wlt, int acceleration)
{
	...
	if (acceleration == 1) {
		...
		printf("Starting Hardware part: %ld\n", sysconf(_SC_PAGE_SIZE));
		...
		uint32_t* registers[NUM_CORES];
		for (int x = 0; x < NUM_CORES; x++) {
			registers[x] = (uint32_t *) mmap(NULL, ADDR_RANGE, PROT_READ|PROT_WRITE, MAP_SHARED,
			 fd, ADDR_BASE + x * ADDR_INC);
			registers[x][15] = image_width;
			registers[x][1] = *((int *)&amount);
			registers[x][2] = *((int *)&scale);
			registers[x][4] = do_scale;
		}
		for (int r = 0; r < image_height; r += NUM_CORES) {
			// Write to BRAM and start quantize
			for (int x = 0; x < NUM_CORES; x++) {
				for(int i = 0; i < image_width; i ++){
					writeToBRAM(i, input[(r + x) * image_width + i], registers[x]);
					registers[x][0] = START_SOFT;
				}
			}

			// Wait for quantize to finish and then read the results from BRAM
			for (int x = 0; x < NUM_CORES; x++) {
				while (registers[x][5] == 0) {
					usleep(1);
				}
				registers[x][0] = STOP_SOFT;
				for(int i = 0 ; i < image_width; i++){
					q[i] = (unsigned char)readFromIntBRAM(i, registers[x]);
				}
				registers[x][0] = RESTART_SOFT;
			}
		}
