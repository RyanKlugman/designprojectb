...
unsigned char *Quantize(float *input, int img_size, int amount, wlt_header_info &wlt, int acceleration)
{
	...
	std::clock_t start;
	std::clock_t bram_write;
	std::clock_t bram_read;
	double duration;
	...
	if (acceleration == 1){
		...
		double bram_read_duration = 0;
		double bram_write_duration = 0;
		double hard_duration = 0;

		for(int r = 0; r < image_height;r+=4) { 
			bram_write = std::clock();
			...
			bram_write_duration += ( std::clock() - bram_write ) / (double) CLOCKS_PER_SEC;
		...
		//	}
			...
			int c = 0;
			// printf("register 5: %d \n",registers[5]);
			while ((registers0[5] == 0) or (registers1[5] == 0) or (registers2[5] == 0) or (registers3[5] == 0)){ //polling for done
				c++;	
			}
			hard_duration += ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
			...
			bram_read_duration += ( std::clock() - bram_read ) / (double) CLOCKS_PER_SEC;
		...
		printf("BRAM Write Duration : %f\n", bram_write_duration);
		printf("BRAM Read Duration : %f\n", bram_read_duration);
		printf("Actual Hardware Duration : %f\n", hard_duration);
		...
		printf("Actual Software Duration : %f\n", duration);
	}
