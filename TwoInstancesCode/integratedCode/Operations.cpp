...
unsigned char *Quantize(float *input, int img_size, int amount, wlt_header_info &wlt, int acceleration)
{
	...
	if(acceleration == 1){
		...
		registers[15] = image_width;

		//Adress 1 hold float Amount
		writeToRegister(1,(float)amount,registers);
		...
		for(int r = 0; r < image_height;r+=2) { 
			...
			for(int i = 0; i < image_width; i ++){
				writeToBRAM(i, input[r * image_width + i],registers);
			}
			registers[14] = 1;
			for(int i = 0; i < image_width; i ++){
				writeToBRAM(i, input[(r + 1) * image_width + i],registers);
			}
			// registers[14] = 0;
			// if(r < 8){
			// 	for(int i = 0 ; i <8; i++){
			// 	 	printf("Read from bram0 :%d value: %f\n",i, readFromBRAM(i,registers));
			// 	}
			// }
			// registers[14] = 1;
			// if(r < 8){
			// 	for(int i = 0 ; i <8; i++){
			// 	 	printf("Read from bram1 :%d value: %f\n",i, readFromBRAM(i,registers));
			// 	}
			// }
			//starting hardware.
			//printf("Checking Register 0 : %d \n",registers[0]);
			registers[0] = START_SOFT;
			int c = 0;
			// printf("register 5: %d \n",registers[5]);
			while ((registers[5] == 0)){ //polling for done
				c++;	
			}
			registers[0] = STOP_SOFT;
			//printf("register 5: %d \n",registers[5]);
			// printf("Counter:%d\n",c);	
			// printf("Done\n");
			registers[14] = 0;
			for(int i = 0 ; i < image_width; i++){
				q[r * image_width + i] = (unsigned char)readFromIntBRAM(i,registers);
			}
			// if(r < 8){
			// 	for(int i = 0 ; i <8; i++){
			// 	 	printf("Read from bram0 :%d value: %d\n",i, readFromIntBRAM(i,registers));
			// 	}
			// }
			registers[14] = 1;
			for(int i = 0 ; i < image_width; i++){
				q[(r + 1) * image_width + i] = (unsigned char)readFromIntBRAM(i,registers);
			}
			// if(r < 8){
			// 	for(int i = 0 ; i <8; i++){
			// 	 	printf("Read from bram1 :%d value: %d\n",i, readFromIntBRAM(i,registers));
			// 	}
			// }
